...
@Component({
        ...
        <div *ngIf="!temCoordenadoresDefinidos">
          <mat-form-field appearance="outline" style="width: 100%; margin-top: 15px;">
            ...
            <input
              ...
              placeholder="Ex: João Silva & Ana Santos"
            />
          </mat-form-field>
        </div>
  ...
  styles: [
    `
      ...
      .mini-phone-input:focus {
        outline: none;
        border-color: #1976d2;
        color: #000;
      }
      .mini-phone-input.missing {
...
export class DialogGerarAcessoComponent implements OnInit {
  ...
  async prepararDestinatarios() {
    ...
    const nomesRaw = this.nomeSelecionado
      .split('&')
      .map((n) => n.trim())
      .filter((n) => n);
